...
def read_file_and_reshape_stl(inputFilePath, resolution):
    """
...
# Jk Addition
def read_tensors_and_reshape_stl(meshTensor: Tensor, resolution):
    """
    Read stl file and reshape mesh
    """
    mesh = meshTensor.cpu().numpy()
    (scale, shift, bounding_box) = mesh_slice.calculateScaleAndShift(mesh, resolution)
    mesh = list(mesh_slice.scaleAndShiftMesh(mesh, scale, shift))
    return (mesh, bounding_box)
...
if __name__ == '__main__':
    ...
    # read and rescale
    mesh, bounding_box = read_file_and_reshape_stl(args.input, args.resolution)
    # create voxel array
